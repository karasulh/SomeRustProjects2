...
fn main()->Result<(),failure::Error>{
    let clap = clap_app!(doodle_cli => 
                            ...
                            (@subcommand new_poll => 
                                (@arg user_id:-u +takes_value "The owner of the poll") //takes_value: can be nullable, not required a owner
                                (@arg question: +required "The question")
                                (@arg options: +required "The options(Comma Seperated)")
                            )
    ...
    if let Some(ref sub) = clap.subcommand_matches("new_user"){
        let user = new_user(sub.value_of("name").unwrap(), sub.value_of("password").unwrap());
        let added: User = diesel::insert_into(users::table).values(&user).get_result(&conn)?;
        println!("New user is added: {:?}",added);
    }
    ...
    if let Some(ref sub) = clap.subcommand_matches("poll_results") {
        ...
        for v in vals {
            if let Some(n) = v.response.selected {
            //if let Some(n) = v.1 {
                let n = n as usize;
                if n < scores.len() {
                    scores[n] += 1;
